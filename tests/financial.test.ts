...
test('DELETE-DATE-01: smart delete can find exact-date cash tracking rows outside recent createdAt window', async () => {
  ...
  assert.ok(serverSrc.includes('batchHasAmbiguousCompleteReport') && serverSrc.includes("effectiveCall.name === 'queryTransactions'") && serverSrc.includes("effectiveCall.name === 'generateReport'"), 'Live must block same-batch camelCase current-cycle reads after ambiguous report calls');
