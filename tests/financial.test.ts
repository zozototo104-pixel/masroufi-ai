...
test('DELETE-DATE-01: smart delete can find exact-date cash tracking rows outside recent createdAt window', async () => {
  ...
  assert.ok(serverSrc.includes('runLiveServerFinancialCompletion') && serverSrc.includes('executing deterministic completion from transcript') && serverSrc.includes('clearLiveServerFinancialCompletionTimer') && serverSrc.includes('completedFromLiveExpenseDraft') && serverSrc.includes('parseArabicAmountWords'), 'Live must deterministically complete pending/drafted financial entries from short transcript replies like سجليه بال باي when Gemini fails to emit a tool call, while still cancelling this fallback if Gemini does emit a financial write tool');
