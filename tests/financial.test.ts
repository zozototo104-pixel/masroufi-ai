...
test('DELETE-DATE-01: smart delete can find exact-date cash tracking rows outside recent createdAt window', async () => {
  ...
  assert.ok(toolsSrc.includes('export async function getRecentTransactions') && toolsSrc.includes('success: true') && toolsSrc.includes('آخر ${transactions.length} عمليات مالية') && toolsSrc.includes('getRecentTransactions: getRecentTransactions'), 'get_recent_transactions must return success/message and support Live camelCase aliases');
