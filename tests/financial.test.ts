...
test('REPORTS-06: Treasurer/monthly expense analysis must not refuse or misroute to today reads', async () => {
  ...
  assert.ok(treasurerSrc.includes("import { matchesArabicCategory } from '../lib/reportUtils'") && treasurerSrc.includes('matchesArabicCategory(t, categoryQuery)') && treasurerSrc.includes('args?.prefiltered === true'), 'treasurer category filtering must use the shared Arabic matcher and must not date-filter already prefiltered salary-cycle rows');
