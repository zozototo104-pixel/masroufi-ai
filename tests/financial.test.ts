...
test('DELETE-DATE-01: smart delete can find exact-date cash tracking rows outside recent createdAt window', async () => {
  ...
  assert.ok(serverSrc.includes('responseModalities: [Modality.AUDIO]') && !serverSrc.includes('inputAudioTranscription') && !serverSrc.includes('[live-fallback]'), 'Live mode must stay audio-only until transcript fallback is implemented safely');
