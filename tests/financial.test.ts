...
test('LIVE-02: Gemini Live quota exhaustion is classified and surfaced to the user', async () => {
  ...
  assert.ok(liveSrc.includes('outputGain.gain.value = 2.15') && liveSrc.includes('outputCompressor') && liveSrc.includes('outputCompressor.disconnect()'), 'client must boost quiet Gemini Live playback loudly with compression without changing microphone input and must clean up audio nodes');
