...
test('DELETE-DATE-01: smart delete can find exact-date cash tracking rows outside recent createdAt window', async () => {
  ...
  assert.ok(toolsSrc.includes('reportIntentText') && toolsSrc.includes('inferredReportMonth') && toolsSrc.includes("period: 'salary_cycle'") && toolsSrc.includes('month: inferredReportMonth'), 'generate_report must infer salary-cycle month from title text like شهر 8 instead of defaulting to the current cycle');
