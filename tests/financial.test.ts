...
test('DELETE-DATE-01: smart delete can find exact-date cash tracking rows outside recent createdAt window', async () => {
  ...
  assert.ok(serverSrc.includes("'get_recent_transactions'") && serverSrc.includes("'getRecentTransactions'") && serverSrc.includes('sanitizeLiveFunctionResponsesForGemini') && serverSrc.includes('summaryOnly') && serverSrc.includes('messageChars') && serverSrc.includes('وجدت ${transactions.length} عملية مالية') && serverSrc.includes('rerouted latest operations read to get_recent_transactions') && serverSrc.includes('لا تستخدم memory_search لهذا السؤال أبداً') && !serverSrc.includes('spokenSummary'), 'Live recent-transaction reads must be sent to Gemini as compact official tool responses only; latest-operation memory/query reads must be rerouted to get_recent_transactions; and no internal args/requestArgs/spokenSummary payloads may close the session');
