...
test('DELETE-DATE-01: smart delete can find exact-date cash tracking rows outside recent createdAt window', async () => {
  ...
  assert.ok(toolsSrc.includes('post_commit_verification_warning') && toolsSrc.includes('BALANCE_EFFECT_NOT_CONFIRMED_AFTER_COMMIT') && toolsSrc.includes('cloudStoragePending: true') && toolsSrc.includes('لم أعتبر العملية مكتملة ولم أؤكد الخصم'), 'add_transaction must not return success=true when post-commit verification cannot confirm the balance effect/cloud save');
  assert.ok(serverSrc.includes('hasFinancialCloudTouch') && serverSrc.includes('transactionCommitted === true') && serverSrc.includes('isFinancialMutationToolName(r?.name)'), 'Live must refresh the financial UI after a partial cloud-touch write without treating it as a successful mutation');
