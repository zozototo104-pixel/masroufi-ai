...
test('DELETE-DATE-01: smart delete can find exact-date cash tracking rows outside recent createdAt window', async () => {
  ...
  assert.ok(serverSrc.includes("'get_recent_transactions'") && serverSrc.includes("'getRecentTransactions'") && serverSrc.includes('spokenSummary') && serverSrc.includes('قل للمستخدم هذا الملخص صوتياً الآن'), 'Live recent-transaction reads must be recognized and sent back with a speakable summary so the assistant does not stay silent');
