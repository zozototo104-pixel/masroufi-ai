...
test('DELETE-DATE-01: smart delete can find exact-date cash tracking rows outside recent createdAt window', async () => {
  ...
  assert.ok(serverSrc.includes('runLiveServerFinancialCompletion') && serverSrc.includes('executing deterministic completion from transcript') && serverSrc.includes('clearLiveServerFinancialCompletionTimer') && serverSrc.includes('completedFromLiveExpenseDraft') && serverSrc.includes('parseArabicAmountWords') && serverSrc.includes('SERVER_TRANSCRIPT_COMPLETION_ALREADY_HANDLED'), 'Live must deterministically complete pending/drafted financial entries from short transcript replies like سجليه بال باي when Gemini fails to emit a tool call, cancel this fallback if Gemini emits a financial write tool, and prevent later Gemini calls from duplicating the server-completed write');
