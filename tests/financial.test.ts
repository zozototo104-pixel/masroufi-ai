...
test('DELETE-DATE-01: smart delete can find exact-date cash tracking rows outside recent createdAt window', async () => {
  ...
  assert.ok(serverSrc.includes("'get_recent_transactions'") && serverSrc.includes("'getRecentTransactions'") && serverSrc.includes('sanitizeLiveFunctionResponsesForGemini') && serverSrc.includes('summaryOnly') && serverSrc.includes('messageChars') && serverSrc.includes('وجدت ${transactions.length} عملية مالية') && !serverSrc.includes('spokenSummary'), 'Live recent-transaction reads must be sent to Gemini as compact official tool responses only, and query_transactions lists must also get a deterministic spoken summary without internal args/requestArgs/spokenSummary payloads that can close the session');
