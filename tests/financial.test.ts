...
test('REPORTS-06: Treasurer/monthly expense analysis must not refuse or misroute to today reads', async () => {
  ...
  assert.ok(serverSrc.includes('liveMonthlyExpenseMisroutedRead') && serverSrc.includes('هدا') && serverSrc.includes('هاد') && serverSrc.includes("name: 'query_transactions'") && serverSrc.includes("period: 'current_salary_cycle'") && serverSrc.includes('liveReadLimit = isSalaryCycleMonthlyRead ? 300 : 40'), 'Live monthly expense questions, including dialectal الشهر هدا/هاد, must be rerouted from get_recent_transactions/today to current salary cycle query_transactions with a large bounded read');
});
