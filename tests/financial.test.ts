...
test('LIVE-02: Gemini Live quota exhaustion is classified and surfaced to the user', async () => {
  ...
  assert.ok(liveSrc.includes('adaptivePlaybackLeadSecondsRef = useRef(0.75)') && liveSrc.includes('nextAdaptiveLeadSeconds') && liveSrc.includes('Math.min(\n              1.45') && liveSrc.includes('Math.max(0.65'), 'client must use an adaptive Live playback cushion for mobile Chrome/WebView jitter instead of the old shallow fixed 0.28s/0.55s buffer');
  assert.ok(liveSrc.includes('live-audio-client-diagnostics') && liveSrc.includes('interArrivalMs') && liveSrc.includes('playbackUnderrunsRef') && liveSrc.includes('audioContextState') && liveSrc.includes('adaptivePlaybackLeadMs') && liveSrc.includes('client_audio_playback_diagnostics'), 'client must log and forward Live audio chunk timing, adaptive buffer, queue underruns, and AudioContext state for diagnosing Chrome audio cutting');
  assert.ok(serverSrc.includes('live-audio-server-diagnostics') && serverSrc.includes('maxClientAudioInterArrivalMs') && serverSrc.includes('liveClientAudioChunksSentToGemini') && serverSrc.includes('maxGeminiAudioInterArrivalMs') && serverSrc.includes('currentTurnMaxGeminiAudioInterArrivalMs') && serverSrc.includes('adaptivePlaybackLeadMs') && serverSrc.includes('client_audio_playback_diagnostics'), 'server must log microphone chunk timing, Gemini output chunk timing per active turn, adaptive Chrome playback buffer, and Chrome playback diagnostics so we can separate internet/WebSocket issues from playback buffering issues');
