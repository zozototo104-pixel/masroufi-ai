...
test('CLARIFICATION-03: payment/account clarification must complete the original financial operation without repeating questions', async () => {
  ...
  assert.ok(toolsSrc.includes('confirmedStructuredPaymentAccount') && toolsSrc.includes('args.disableExpenseSplitParsing === true'), 'each split expense commit must keep its own cash/PalPay/debt account instead of being overridden by the combined clarification text');
