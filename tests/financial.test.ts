...
test('DELETE-DATE-01: smart delete can find exact-date cash tracking rows outside recent createdAt window', async () => {
  ...
  assert.ok(serverSrc.includes('inputAudioTranscription') && serverSrc.includes('extractLiveInputTranscriptText') && serverSrc.includes('shouldRunServerSideLiveWriteFallback'), 'Live mode must capture user speech so skipped write tools can be corrected server-side');
  assert.ok(serverSrc.includes('[live-fallback] executing deterministic mutation') && serverSrc.includes('buildFallbackFinancialToolCall(textForFallback') && serverSrc.includes('serverSideSaved'), 'Live mode must execute deterministic fallback when Gemini says it handled a write but did not call a mutation tool');
