...
test('LIVE-02: Gemini Live quota exhaustion is classified and surfaced to the user', async () => {
  ...
  assert.ok(liveSrc.includes('live-audio-client-diagnostics') && liveSrc.includes('interArrivalMs') && liveSrc.includes('playbackUnderrunsRef') && liveSrc.includes('audioContextState'), 'client must log Live audio chunk timing, queue underruns, and AudioContext state for diagnosing Chrome audio cutting without changing playback behavior');
  assert.ok(serverSrc.includes('live-audio-server-diagnostics') && serverSrc.includes('maxClientAudioInterArrivalMs') && serverSrc.includes('liveClientAudioChunksSentToGemini') && serverSrc.includes('maxGeminiAudioInterArrivalMs'), 'server must log microphone chunk timing and Gemini output chunk timing so we can separate internet/WebSocket issues from playback buffering issues');
