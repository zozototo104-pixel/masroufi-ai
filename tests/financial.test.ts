...
test('DELETE-DATE-01: smart delete can find exact-date cash tracking rows outside recent createdAt window', async () => {
  ...
  assert.ok(toolsSrc.includes('export async function getRecentTransactions') && toolsSrc.includes('success: true') && toolsSrc.includes('آخر ${transactions.length} عمليات مالية') && toolsSrc.includes('getRecentTransactions: getRecentTransactions') && toolsSrc.includes('requestedDateKey') && toolsSrc.includes('createdLocalDateKey') && toolsSrc.includes('bounded_user_query_filtered_by_date_or_createdAt'), 'get_recent_transactions must return success/message, support Live camelCase aliases, and answer today/latest registered operations by checking both transaction date and createdAt');
