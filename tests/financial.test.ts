...
test('DELETE-DATE-01: smart delete can find exact-date cash tracking rows outside recent createdAt window', async () => {
  ...
  assert.ok(serverSrc.includes('pendingFinancialClarifications') && serverSrc.includes('buildPendingFinancialClarificationCall') && serverSrc.includes('completedPendingClarification') && serverSrc.includes('clarificationReplyText'), 'server must remember incomplete financial operations and complete them from short replies like كاش/PalPay/دين');
  assert.ok(serverSrc.includes('completing pending clarification from Live transcript') && serverSrc.includes('rememberLiveFinancialCommit(liveFinancialCommitKey({ ...pendingClarificationCall'), 'Live should complete a stored financial clarification when the user answers كاش/PalPay/دين and protect against duplicate follow-up tool calls');
