...
test('LIVE-02: Gemini Live quota exhaustion is classified and surfaced to the user', async () => {
  ...
  assert.ok(liveSrc.includes('live-audio-client-diagnostics') && liveSrc.includes('interArrivalMs') && liveSrc.includes('playbackUnderrunsRef') && liveSrc.includes('audioContextState') && liveSrc.includes('client_audio_playback_diagnostics'), 'client must log and forward Live audio chunk timing, queue underruns, and AudioContext state for diagnosing Chrome audio cutting without changing playback behavior');
  assert.ok(serverSrc.includes('live-audio-server-diagnostics') && serverSrc.includes('maxClientAudioInterArrivalMs') && serverSrc.includes('liveClientAudioChunksSentToGemini') && serverSrc.includes('maxGeminiAudioInterArrivalMs') && serverSrc.includes('client_audio_playback_diagnostics'), 'server must log microphone chunk timing, Gemini output chunk timing, and Chrome playback diagnostics so we can separate internet/WebSocket issues from playback buffering issues');
