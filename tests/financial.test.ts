...
test('DOMAIN-03B: credit purchases accept creditor alias and do not require cash preflight', async () => {
  ...
  assert.ok(toolsSrc.includes('structuredCreditPurchaseIntent') && toolsSrc.includes("String(args.transactionType || '').toUpperCase() === 'CREDIT_PURCHASE'"), 'structured creditor/payment fields from Gemini voice must force credit purchases to account=debt even if userText is missing');
  ...
  assert.ok(toolsSrc.includes('transactionId = String(args?.transactionId') && toolsSrc.includes('direct_selected_credit_purchase_repair'), 'repair must support direct selected transactionId conversion without relying on assistant recognition');
  ...
  assert.ok(serverSrc.includes('/api/transactions/repair-credit-purchase'), 'UI/direct repair must have an API endpoint independent of Gemini recognition');
...
test('DOMAIN-03B2: salary cycle details expose credit purchases under a debt bucket', async () => {
  ...
  assert.ok(appSrc.includes('repairActualBalanceSnapshot') && appSrc.includes('إصلاح الرصيد الفعلي'), 'UI must expose a manual account-balance snapshot repair for mismatches between actual balance and cycle spendable');
});
...
test('DELETE-DATE-01: smart delete can find exact-date cash tracking rows outside recent createdAt window', async () => {
  ...
  assert.ok(serverSrc.includes('mutationNames') && serverSrc.includes('isReadSideCall') && serverSrc.includes('duplicateSideCall'), 'Live normalization must not mask distinct failed write calls');
  assert.ok(serverSrc.includes('responseModalities: [Modality.AUDIO]') && serverSrc.includes('inputAudioTranscription: {}') && serverSrc.includes('recordLiveExpenseIntakeTranscript(inputTranscript)') && serverSrc.includes('scheduleLiveServerFinancialCompletion(inputTranscript)'), 'Live mode must expose input transcripts to the server-side financial clarification guard so short replies like بال باي complete the pending purchase instead of repeating the same question');
  assert.ok(serverSrc.includes('recentLiveFinancialInFlight') && serverSrc.includes('duplicate mutation joined existing in-flight write') && serverSrc.includes('joinedInFlight'), 'Live mode must join concurrent duplicate mutation calls instead of saving the same purchase twice');
  assert.ok(serverSrc.includes('FINANCIAL_MUTATION_TOOL_NAMES') && serverSrc.includes('looksLikeFinancialWriteIntent') && serverSrc.includes('!hasFinancialMutationToolCall'), 'text fallback must run when a write intent produced only read tools such as query_transactions');
  assert.ok(serverSrc.includes("'get_recent_transactions'") && serverSrc.includes("'getRecentTransactions'") && serverSrc.includes('sanitizeLiveFunctionResponsesForGemini') && serverSrc.includes('compactLiveToolResponseForGemini') && serverSrc.includes('summaryOnly') && serverSrc.includes('messageChars') && serverSrc.includes('وجدت ${transactions.length} عملية مالية') && serverSrc.includes('rerouted latest operations read to get_recent_transactions') && serverSrc.includes('wantsTodayRecent') && serverSrc.includes('لا تستخدم memory_search لهذا السؤال أبداً') && !serverSrc.includes('spokenSummary'), 'Live recent-transaction reads must be sent to Gemini as compact official tool responses only; latest-operation memory/query reads must be rerouted to get_recent_transactions with today scope; and no internal args/requestArgs/spokenSummary payloads may close the session');
  assert.ok(serverSrc.includes('postToolAudioFallbackTimer') && serverSrc.includes('sendClientContent') && serverSrc.includes('post-tool audio fallback prompting Gemini to speak deterministic summary') && serverSrc.includes('held mic chunk while waiting for post-tool answer audio') && serverSrc.includes('completedPostToolTurnWithoutAudio'), 'Live read tools must not get stuck after thinking; they need a post-tool spoken fallback, must survive silent turnComplete, and need a short mic gate while waiting for audio');
  assert.ok(toolsSrc.includes('export async function getRecentTransactions') && toolsSrc.includes('success: true') && toolsSrc.includes('آخر ${transactions.length} عمليات مالية') && toolsSrc.includes('getRecentTransactions: getRecentTransactions') && toolsSrc.includes('requestedDateKey') && toolsSrc.includes('createdLocalDateKey') && toolsSrc.includes('targeted_createdAt_or_date_day_range_then_fallback') && toolsSrc.includes("where('createdAt', '>=', startUtcIso)") && toolsSrc.includes("where('date', '>=', requestedDateKey)") && toolsSrc.includes("typeof value?.toDate === 'function'") && toolsSrc.includes('timestampMs'), 'get_recent_transactions must return success/message, support Live camelCase aliases, and answer today/latest registered operations by targeted createdAt/date day-range reads plus local-day matching, including Firestore Timestamp values');
  assert.ok(serverSrc.includes('أمر شراء مكتمل في الصوت') && serverSrc.includes('تنفيذ add_transaction فوراً') && serverSrc.includes('ممنوع تبدأ بـ query_transactions'), 'Live instructions must require add_transaction before read tools for completed purchases');
  assert.ok(toolsSrc.includes('هذه الأداة هي المسار الصحيح فوراً وليست query_transactions') && toolsSrc.includes('required: ["amount", "type", "account", "category", "subcategory", "paymentMethod"]'), 'add_transaction declaration must force account/payment mapping for completed purchases');
  assert.ok(serverSrc.includes('pendingFinancialClarifications') && serverSrc.includes('buildPendingFinancialClarificationCall') && serverSrc.includes('completedPendingClarification') && serverSrc.includes('clarificationReplyText'), 'chat must remember incomplete financial operations and complete them from short replies like كاش/PalPay/دين');
  assert.ok(serverSrc.includes('sanitizePendingFinancialArgs') && serverSrc.includes('previousArgs') && serverSrc.includes('INVALID_AMOUNT') && serverSrc.includes('cleanFinancialClarificationText') && serverSrc.includes('applyExpenseInferenceToPatch') && serverSrc.includes('isAmountOnlyClarificationAnswer'), 'financial clarification memory must accumulate split answers like merchant → item → amount → payment, must not store spoken amounts as items, and must not reuse failed operation ids');
  assert.ok(serverSrc.includes('pendingClarificationAnswer') && serverSrc.includes('liveExpenseIntakeDraft.args') && serverSrc.includes('merchant: liveExpenseIntakeDraft.args.merchant') && serverSrc.includes('purchaseItem: liveExpenseIntakeDraft.args.purchaseItem'), 'Live deterministic fallback must keep a multi-turn expense draft, not only the final cash/PalPay/debt word');
  assert.ok(serverSrc.includes('mergePendingFinancialClarificationIntoToolCall') && serverSrc.includes('updatePendingFinancialClarificationFromResponses(userId, functionResponses as any') && serverSrc.includes('hasClarificationMerge') && serverSrc.includes('clarificationReplyText: existingArgs.clarificationReplyText || liveCurrentUserText') && serverSrc.includes('إجابات التوضيح') && serverSrc.includes('أعد استدعاء الأداة المالية نفسها بالقيم السابقة مع الإجابة الجديدة'), 'Live must store pending financial clarification from tool responses, instruct the model to execute the tool after short replies, and merge a short PalPay/cash/debt reply into the next tool call while preserving the original purchase text');
  assert.ok(serverSrc.includes('runLiveServerFinancialCompletion') && serverSrc.includes('executing deterministic completion from transcript') && serverSrc.includes('clearLiveServerFinancialCompletionTimer') && serverSrc.includes('completedFromLiveExpenseDraft') && serverSrc.includes('parseArabicAmountWords') && serverSrc.includes('SERVER_TRANSCRIPT_COMPLETION_ALREADY_HANDLED') && serverSrc.includes('SERVER_TRANSCRIPT_COMPLETION_IN_PROGRESS') && serverSrc.includes('liveServerFinancialCompletionBlockUntilMs'), 'Live must deterministically complete pending/drafted financial entries from short transcript replies like سجليه بال باي when Gemini fails to emit a tool call, cancel this fallback if Gemini emits a financial write tool, and prevent late Gemini calls from duplicating the server-completed write even if its generated details differ');
  assert.ok(toolsSrc.includes('args.userText,') && toolsSrc.includes('args.currentUserText,') && toolsSrc.includes('args.clarificationReplyText,'), 'add_transaction must validate account intent using both the original utterance and the clarification reply so PalPay/cash/debt answers are not lost');
  assert.ok(serverSrc.includes('Let fallback create clarification-producing calls') || (serverSrc.includes('...(account ? { paymentMethod: account, account } : {})') && serverSrc.includes('if (isPurchase)')), 'fallback purchase parser must create an add_transaction call even when payment method is missing so clarification memory can be stored');
  assert.ok(toolsSrc.includes("reason:'MISSING_DEBT_PAYMENT_ACCOUNT'") && toolsSrc.includes("missingFields:['debtPaymentAccount']") && !toolsSrc.includes("args.paymentMethod||args.fromAccount||'cash'"), 'pay_debt must ask for cash/PalPay instead of assuming cash');
  assert.ok(toolsSrc.includes("reason: 'INVALID_AMOUNT'") && toolsSrc.includes("missingFields: ['amount']") && toolsSrc.includes('ما قيمة التحويل أو الدين بالضبط؟'), 'transfer/debt flows must ask for a missing amount so clarification memory can continue instead of dropping the operation');
  assert.ok(toolsSrc.includes('MISSING_BORROW_DESTINATION') && toolsSrc.includes("missingFields: ['borrowDestination']") && toolsSrc.includes('Do not assume cash destination') || toolsSrc.includes('استلمت الدين كاش أم في محفظة PalPay؟'), 'borrowing money from debt must ask where the money was received instead of assuming cash');
  ...
  assert.ok(serverSrc.includes('تقرير كامل/شامل/مفصل/كل المصروفات') && serverSrc.includes('استخدم generate_report') && serverSrc.includes('ولا ترفض بسبب كثرة البيانات'), 'complete written expense report requests must be routed to generate_report and not refused due to size');
  assert.ok(toolsSrc.includes('requestedFullWrittenReport') && toolsSrc.includes('تم إنشاء التقرير الكتابي الكامل وحفظه في حافظة التقارير') && !toolsSrc.includes("reason: 'REPORT_QUERY_INCOMPLETE'"), 'generate_report must save complete written reports instead of returning REPORT_QUERY_INCOMPLETE for long periods');
  assert.ok(toolsSrc.includes('reportUserIntentText') && toolsSrc.includes('monthFromUserText') && toolsSrc.includes('monthFromTitle') && toolsSrc.includes('const inferredReportMonth = monthFromUserText ?? monthFromTitle ?? explicitReportMonth'), 'generate_report must prioritize the user wording month like شهر 8 over a wrong generated title/current-cycle month');
  assert.ok(toolsSrc.includes('REPORT_SCOPE_MONTH_REQUIRED') && toolsSrc.includes('لا يجوز إنشاء تقرير للدورة الحالية افتراضياً') && serverSrc.includes('liveReportScopeMissingUntilMs') && serverSrc.includes('طلب التقرير السابق لم يصل معه شهر الدورة'), 'ambiguous complete reports must not fall back to vault_2026_09/current_salary_cycle when month is missing');
  assert.ok(serverSrc.includes('batchHasAmbiguousCompleteReport') && serverSrc.includes("effectiveCall.name === 'queryTransactions'") && serverSrc.includes("effectiveCall.name === 'generateReport'") && serverSrc.includes('ambiguous complete report scope detected before tool execution'), 'Live must block same-batch camelCase current-cycle reads after ambiguous report calls');
  assert.ok(toolsSrc.includes('queryTransactions: queryTransactions') && toolsSrc.includes('generateReport: generateReport'), 'Live camelCase report/read tool aliases must route through the same guarded handlers');
  assert.ok(serverSrc.includes('[REPORT_SCOPE_TRACE] api_reports_generate_in') && serverSrc.includes('[REPORT_SCOPE_TRACE] live_tool_before') && toolsSrc.includes('[REPORT_SCOPE_TRACE] generate_report_range_resolved') && toolsSrc.includes('reportScopeTrace'), 'report scope tracing must expose where month 8 becomes month 9');
  assert.ok(serverSrc.includes('liveReportScopeOverride') && serverSrc.includes('live_report_scope_override_set') && serverSrc.includes('live_query_scope_overridden_from_report') && serverSrc.includes("period: 'salary_cycle'"), 'after a Live report with month=8, follow-up current_salary_cycle reads must be forced to the report month instead of active month 9');
  assert.ok(toolsSrc.includes('reportScope: reportToSave.reportScope') && !toolsSrc.includes('salaryCycle: reportToSave.salaryCycle,\n    reportScopeTrace,'), 'generate_report must not return raw trace/current active month 9 context to the assistant');
  assert.ok(appSrc.includes('titleLooksLikeScopeReport') && appSrc.includes('تقرير|مصروفات|شهر|دورة|راتب') && appSrc.includes('explicitCategory'), 'report display/print must not treat a month report title like تقرير مصروفات شهر 8 as a category');
  assert.ok(toolsSrc.includes('exactDateShouldOverrideCycle') && toolsSrc.includes('readTransactionsForSmartDeleteDate(explicitDateKey') && toolsSrc.includes('transactionDateKey(t) === explicitDateKey'), 'exact date queries like 30/8 must not be interpreted as salary-cycle month 8 and must use normalized local-day matching');
  ...
  assert.ok(toolsSrc.includes('verifyAddTransactionCommit') && toolsSrc.includes('TRANSACTION_DOC_NOT_FOUND_AFTER_COMMIT') && toolsSrc.includes('BALANCE_SNAPSHOT_MISMATCH_AFTER_COMMIT'), 'add_transaction must verify/repair the ledger document and balance snapshot after commit');
  assert.ok(toolsSrc.includes('advisory; they must never block editing an existing transaction') && toolsSrc.includes('budgetWarning') && toolsSrc.includes('تم تنفيذ التعديل لأن تحذير الميزانية لا يمنع تعديل عملية موجودة') && !toolsSrc.includes("reason: 'BUDGET_CHECK_BOUNDED_QUERY_UNCERTAIN'"), 'update_transaction budget checks must be advisory and must not block edits');
  assert.ok(toolsSrc.includes('optional budget/income preflight was partial; continuing to atomic write path') && !toolsSrc.includes("message: 'تعذّر التحقق من مؤشرات الميزانية/الدخل بدقة. لن أنفذ العملية المالية من بيانات جزئية.'"), 'optional budget/income preflight partials must not block add_transaction');
  assert.ok(toolsSrc.includes('advisoryWarnings.push(`تحذير ميزانية') && toolsSrc.includes('advisoryWarnings.push(`تحذير أمين الصندوق') && !toolsSrc.includes("reason:'BUDGET_WILL_BE_EXCEEDED'") && !toolsSrc.includes("reason:'TREASURER_RISK_REVIEW_REQUIRED'"), 'add_transaction budget/treasurer warnings must not block clear expense writes');
  assert.ok(toolsSrc.includes('post_commit_verification_warning') && toolsSrc.includes('تم تنفيذ Firestore transaction بنجاح، لكن فحص ما بعد الحفظ سجل تحذيراً داخلياً') && !toolsSrc.includes('BALANCE_EFFECT_NOT_CONFIRMED_AFTER_COMMIT') && !toolsSrc.includes('لم أعتبر العملية مكتملة ولم أؤكد الخصم'), 'add_transaction must not downgrade a successful Firestore transaction because of a post-commit verification warning; atomic write is the source of truth');
  assert.ok(serverSrc.includes('hasFinancialCloudTouch') && serverSrc.includes('transactionCommitted === true') && serverSrc.includes('isFinancialMutationToolName(r?.name)'), 'Live must refresh the financial UI after financial writes, including writes that reached the cloud and had advisory verification warnings');
...
test('LIVE-02: Gemini Live quota exhaustion is classified and surfaced to the user', async () => {
  ...
  assert.ok(liveSrc.includes('outputGain.gain.value = 2.15') && liveSrc.includes('outputCompressor') && liveSrc.includes('outputCompressor.disconnect()') && liveSrc.includes('ensureOutputChain(outputCtx)') && liveSrc.includes('source.connect(outputGain)'), 'client must boost quiet Gemini Live playback loudly through one persistent compressed output chain without changing microphone input and must clean up audio nodes');
  assert.ok(liveSrc.includes('adaptivePlaybackLeadSecondsRef = useRef(1.25)') && liveSrc.includes('nextAdaptiveLeadSeconds') && liveSrc.includes('Math.min(\n              2.4') && liveSrc.includes('Math.max(1.05') && liveSrc.includes('maximumLeadTimeSeconds = 6.0') && liveSrc.includes('currentTime + 0.02'), 'client must use a deeper adaptive Live playback cushion only at queue start/true underrun instead of pushing every chunk and creating artificial gaps');
  assert.ok(liveSrc.includes('isLiveAudioClientDiagnosticsEnabled') && liveSrc.includes('client_audio_playback_diagnostics') && serverSrc.includes('LIVE_AUDIO_DIAGNOSTICS_ENABLED'), 'Live audio diagnostics must be opt-in so logging itself does not overload Render/WebSocket during normal playback');
  assert.ok(!liveSrc.includes('rms > 0.08') && !liveSrc.includes('userSpeechCounter'), 'client must not auto-interrupt expert playback from microphone RMS because speaker echo can chop the expert voice');
...
test('REPORTS-06: Treasurer/monthly expense analysis must not refuse or misroute to today reads', async () => {
  const toolsSrc = await import('node:fs/promises').then(fs => fs.readFile(join(process.cwd(), 'src/server/tools.ts'), 'utf8'));
  ...
  assert.ok(toolsSrc.includes('readTransactionsForSalaryCycle(salaryCycleForTreasurer') && toolsSrc.includes('prefiltered: true') && toolsSrc.includes('treasurerMonthFromText'), 'treasurer salary-cycle/top-expense analysis must use the same safe salary-cycle reader and must preserve explicit spoken months like دورة شهر تسعة');
  assert.ok(toolsSrc.includes('else if (!salaryCyclePeriod)') && toolsSrc.includes('Salary-cycle reads are already filtered by transactionDateKey'), 'query_transactions must not erase salary-cycle rows by re-filtering Firestore Timestamp dates with String(t.date)');
  assert.ok(treasurerSrc.includes("import { matchesArabicCategory } from '../lib/reportUtils'") && treasurerSrc.includes('matchesArabicCategory(t, categoryQuery)') && treasurerSrc.includes('args?.prefiltered === true') && treasurerSrc.includes('transactionTimeMs') && treasurerSrc.includes('toDate'), 'treasurer category/date filtering must use the shared Arabic matcher, must not date-filter already prefiltered salary-cycle rows, and must understand Firestore Timestamp dates');
  assert.ok(salaryCycleSrc.includes('ARABIC_MONTH_NUMBER_WORDS') && salaryCycleSrc.includes('matchedNumberWord'), 'salary-cycle parsing must understand spoken Arabic month numbers like تسعة/تاسع');
  assert.ok(serverSrc.includes('liveMonthlyExpenseMisroutedRead') && serverSrc.includes('liveExplicitSalaryCycleExpenseQuestion') && serverSrc.includes('salaryCycleMonthFromText') && serverSrc.includes('هدا') && serverSrc.includes('هاد') && serverSrc.includes("name: 'query_transactions'") && serverSrc.includes("period: targetSalaryCyclePeriod") && serverSrc.includes('liveReadLimit = isSalaryCycleMonthlyRead ? 300 : 40'), 'Live monthly and explicit salary-cycle expense questions, including الشهر هدا/هاد and دورة شهر تسعة, must be rerouted from get_recent_transactions/today to query_transactions with the requested salary-cycle month preserved');
...
test('CLARIFICATION-03: payment/account clarification must complete the original financial operation without repeating questions', async () => {
  ...
  assert.ok(serverSrc.includes('purchaseItemClarifiedByUser') && serverSrc.includes('beneficiaryClarifiedByUser') && serverSrc.includes('necessityClarifiedByUser'), 'short clarification answers for item/purpose/necessity must be marked as trusted user-provided details');
  ...
  assert.ok(serverSrc.includes('if (!patch && isBareConfirmationAnswer') && serverSrc.includes('execute the original pending operation using the accumulated details') && serverSrc.includes('patch || {}'), 'final save confirmations like سجلها/تمام must execute the accumulated pending operation instead of dropping the save');
  assert.ok(serverSrc.includes('Do NOT mark this semantic operation as handled before the tool commits') && serverSrc.includes('liveServerFinancialCompletionKeys.set(liveKey, Date.now())'), 'Live deterministic completion must only mark an operation handled after a confirmed cloud commit');
  assert.ok(serverSrc.includes('}, 250);'), 'Live clarification answers must trigger deterministic financial completion quickly before the model can claim success without a server write');
  ...
  assert.ok(toolsSrc.includes('confirmedStructuredPaymentAccount') && toolsSrc.includes('args.disableExpenseSplitParsing === true') && toolsSrc.includes('originalTextHasMixedPaymentMethods') && toolsSrc.includes('splitOrMixedStructuredPaymentAccount'), 'each split expense commit must keep its own cash/PalPay/debt account instead of being overridden by the combined clarification text');
  assert.ok(toolsSrc.includes('splitPaymentLooksAlreadyComponentized'), 'when Gemini already sends separate cash and PalPay add_transaction calls, the server must not split each component again');
  assert.ok(serverSrc.includes('...pendingArgs,\n    ...args,\n    ...actualPatch'), 'pending clarification merge must preserve the fresh tool call amount/account so cash and PalPay split retries are not collapsed into one pending operation');
