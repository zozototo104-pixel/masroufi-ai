...
test('DELETE-DATE-01: smart delete can find exact-date cash tracking rows outside recent createdAt window', async () => {
  ...
  assert.ok(serverSrc.includes("'get_recent_transactions'") && serverSrc.includes("'getRecentTransactions'") && serverSrc.includes('sanitizeLiveFunctionResponsesForGemini') && serverSrc.includes('summaryOnly') && serverSrc.includes('messageChars'), 'Live recent-transaction reads must be sent to Gemini as compact official tool responses only, without internal args/requestArgs/spokenSummary payloads that can close the session');
