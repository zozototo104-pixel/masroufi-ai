...
export function useGeminiLive(settings?: { voice: string; persona: string; apiKey: string; idToken: string | null; userName: string; aiName: string; relationship?: string; activeSalaryCycleId?: string; activeSalaryCycleName?: string; activeSalaryCycleMonth?: number; activeSalaryCycleYear?: number }) {
  ...
  const voicedAudioFramesRef = useRef(0);
  ...
  const adaptivePlaybackLeadSecondsRef = useRef(1.25);
  ...
  }, [clearLiveReadyWatchdog, clearResponseWatchdog]);
    ...
    try {
      ...
      ws.onmessage = async (event) => {
        ...
        if (msg.audio && outputCtxRef.current) {
          ...
          const outputGain = ensureOutputChain(outputCtxRef.current);
  ...
  }, [clearLiveReadyWatchdog, clearResponseWatchdog, disconnect, ensureOutputChain, stopPlayback]);
