...
import { calculateBalances, normalizeAccount, normalizeCreditorKey } from '../lib/balanceCalc';
...
type PreparedImportedTransaction = { sourceId: string; docData: any };
...
function prepareImportedFinancialTransactions(transactions: any[], userId: string): {
  ok: true;
  ...
  for (const [index, t] of transactions.entries()) {
    ...
    if (!docData.creditorKey && creditor) docData.creditorKey = normalizeCreditorName(creditor);
