...
async function commitSalaryCycleAndVaultMeta(args: any, userId: string, period: SalaryCyclePeriod, summary: any, readResult: any) {
  ...
  return firebaseAdminDb.runTransaction(async (tx: any) => {
    ...
    const record = {
      ...
      sourceVersion: stableDocId(`${period.cycleId}:${summary.totalIncome}:${summary.totalInflow}:${summary.debtCashInflow}:${summary.totalExpense}:${summary.transactionCount}:${nextVaultContribution}`),
      adjustments,
...
function normalizeVaultCurrency(currency: any): string {
  const cur = String(currency || 'ILS').trim().toUpperCase();
  if (cur === '₪' || cur === 'NIS' || cur === 'شيكل' || cur === 'شيكل إسرائيلي') return 'ILS';
  if (cur === '(args: any, userId: string, token: string) {
  const adminDb = getDb(token);
...
export async function syncOfflineData(args: any, userId: string, token: string) {
 ...
 || cur === 'دولار' || cur === 'دولار أمريكي') return 'USD';
  if (cur === '€' || cur === 'يورو') return 'EUR';
  return cur || 'ILS';
}

function addCurrencyAmount(map: Record<string, number>, currency: string, amount: number): Record<string, number> {
  return { ...map, [currency]: roundMoney(Number(map?.[currency] || 0) + amount) };
}

function normalizeVaultAdjustmentEntries(args: any): Array<{ amount: number; currency: string; source: string; notes: string }> {
  const rawEntries = Array.isArray(args?.amounts) ? args.amounts : Array.isArray(args?.entries) ? args.entries : [{ amount: args?.amount, currency: args?.currency, source: args?.source || args?.reason, notes: args?.notes }];
...
export async function addSavingsVaultAdjustment(args: any, userId: string, token: string) {
  ...
  const convertedEntries = entries.map((entry) => {
    const normalizedAmountIls = normalizeCurrencyToIls(entry.amount, entry.currency);
    const fx = getFxConversionMetadata(entry.amount, entry.currency);
    return { ...entry, normalizedAmountIls, ...fx };
  });
  const missingFx = convertedEntries.find((entry) => entry.currency !== 'ILS' && (!Number.isFinite(Number(entry.normalizedAmountIls)) || Number(entry.normalizedAmountIls) <= 0));
  if (missingFx) {
    return { success: false, retryable: true, reason: 'VAULT_FX_RATE_UNAVAILABLE', message: `لا أستطيع حفظ ${missingFx.amount} ${missingFx.currency} في الخزنة الآن لأن سعر الصرف للشيكل غير متاح. لن أخزن قيمة تقديرية مخترعة.` };
  }
  const source = String(args.source || args.reason || convertedEntries[0]?.source || 'manual_carryover').trim();
  const notes = String(args.notes || convertedEntries.map((e) => e.notes).filter(Boolean).join(' | ') || '').trim();
  const operationSeed = convertedEntries.map((e) => `${e.amount}:${e.currency}:${e.source}`).join('|');
  ...
  const metaRef = firebaseAdminDb.collection('users').doc(userId).collection('meta').doc('savingsVault');
  const now = new Date().toISOString();
  const amountIlsEquivalent = roundMoney(convertedEntries.reduce((sum, entry) => sum + Number(entry.normalizedAmountIls || 0), 0));
  const currencyDelta = convertedEntries.reduce((map: Record<string, number>, entry) => addCurrencyAmount(map, entry.currency, entry.amount), {});
  const result = await firebaseAdminDb.runTransaction(async (tx: any) => {
    const existingAdjustmentSnap = await tx.get(adjustmentRef as any);
    const metaSnap = await tx.get(metaRef as any);
    const metaData = metaSnap.exists ? (metaSnap.data() || {}) : {};
    if (existingAdjustmentSnap.exists) {
      const existing = existingAdjustmentSnap.data() || {};
      return { replay: true, vaultBalance: roundMoney(Number(metaData.currentBalance ?? existing.cumulativeVaultBalance ?? amountIlsEquivalent)), vaultBalanceByCurrency: metaData.balanceByCurrency || existing.cumulativeBalanceByCurrency || {}, adjustment: existing };
    }
    ...
    const nextByCurrency = convertedEntries.reduce((map: Record<string, number>, entry) => addCurrencyAmount(map, entry.currency, entry.amount), previousByCurrency);
    const adjustment = {
      userId,
  ...
  const details = convertedEntries.map((entry) => `${entry.amount} ${entry.currency}`).join(' + ');
  return {
    success: true,
    id: adjustmentId,
...
export async function addSavingsVaultAdjustment(args: any, userId: string, token: string) {
  ...
  const convertedEntries = entries.map((entry) => {
    const normalizedAmountIls = normalizeCurrencyToIls(entry.amount, entry.currency);
    const fx = getFxConversionMetadata(entry.currency);
    return { ...entry, normalizedAmountIls, ...fx };
  });
  const missingFx = convertedEntries.find((entry) => entry.currency !== 'ILS' && (!Number.isFinite(Number(entry.normalizedAmountIls)) || Number(entry.normalizedAmountIls) <= 0));
  if (missingFx) {
...
function deriveAdjustmentCurrencyDelta(adjustment: any): Record<string, number> {
  const explicitDelta = adjustment?.currencyDelta;
  if (explicitDelta && typeof explicitDelta === 'object') {
    return Object.entries(explicitDelta).reduce((map: Record<string, number>, [currency, amount]) => addCurrencyAmount(map, normalizeVaultCurrency(currency), Number(amount || 0)), {});
  }
  const entries = Array.isArray(adjustment?.entries) ? adjustment.entries : [];
  if (entries.length) {
    return entries.reduce((map: Record<string, number>, entry: any) => addCurrencyAmount(map, normalizeVaultCurrency(entry?.currency), parsePositiveFinancialAmount(entry?.amount)), {});
  }
  const currency = normalizeVaultCurrency(adjustment?.originalCurrency || adjustment?.currency || 'ILS');
  const amount = parsePositiveFinancialAmount(adjustment?.originalAmount ?? adjustment?.amount);
  return amount > 0 ? addCurrencyAmount({}, currency, amount) : {};
}

function mergeCurrencyDeltas(base: Record<string, number>, delta: Record<string, number>): Record<string, number> {
  return Object.entries(delta || {}).reduce((map: Record<string, number>, [currency, amount]) => addCurrencyAmount(map, normalizeVaultCurrency(currency), Number(amount || 0)), base || {});
}
...
export async function addSavingsVaultAdjustment(args: any, userId: string, token: string) {
  ...
  const result = await firebaseAdminDb.runTransaction(async (tx: any) => {
    ...
    const previousByCurrency = (metaData.balanceByCurrency || {}) as Record<string, number>;
    const nextBalance = roundMoney(previousBalance + amountIlsEquivalent);
    const nextByCurrency = mergeCurrencyDeltas(previousByCurrency, currencyDelta);
    const adjustment = {
      userId,
      amount: amountIlsEquivalent,
      amountIlsEquivalent,
...
export async function repairSavingsVaultMeta(args: any, userId: string, token: string) {
  ...
  const repairedBalanceByCurrency = manualAdjustments.reduce((map: Record<string, number>, adjustment: any) => mergeCurrencyDeltas(map, deriveAdjustmentCurrencyDelta(adjustment)), cycleVaultTotal > 0 ? { ILS: cycleVaultTotal } : {});
  const metaRef = firebaseAdminDb.collection('users').doc(userId).collection('meta').doc('savingsVault');
  await firebaseAdminDb.runTransaction(async (tx: any) => {
    tx.set(metaRef as any, {
      userId,
