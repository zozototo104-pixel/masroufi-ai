...
function isRecomputableAdvisorAlert(alert: any): boolean {
  const source = String(alert?.source || '').toLowerCase();
  const category = String(alert?.category || '').toLowerCase();
  // These alerts are generated by advisor engines from calculations that the
  // audit already recomputes in the same run. Counting old copies as fresh
  // critical findings creates a feedback loop: audit -> critical alert -> next
  // audit becomes critical only because the old alert is still open.
  return category.startsWith('audit_')
    || ['runfinancialaudit', 'forecastmonthendfinancialposition', 'generatedailyfinancialpulse', 'generateweeklyfinancialrecommendations', 'generateadaptivebudgetplan'].includes(source)
    || ['month_end_forecast', 'daily_financial_pulse', 'weekly_recommendation_plan', 'adaptive_budget_plan'].includes(category);
}
...
export async function reviewRecurringCommitments(args: any, userId: string, token: string) {
  ...
  const commitmentDueKey = (c: any) => normalizeCommitmentDueDateValue(c.dueDate ?? c.dueDayOfMonth ?? c.dueDay, safeNow, null) || auditDateKey(c.dueDate);
  const activeRecurring = commitments.filter((c: any) => {
    ...
    // need due-soon and overdue review if they have a valid due date/day.
    return Boolean(commitmentDueKey(c) || c.recurring || c.recurringFrequency || c.recurringDetectionKey);
  });
  const dueSoon = activeRecurring.filter((c: any) => {
    const dueKey = commitmentDueKey(c);
