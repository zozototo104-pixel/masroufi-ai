...
export async function syncOfflineData(args: any, userId: string, token: string) {
 ...
 || cur === 'دولار' || cur === 'دولار أمريكي' || cur === 'USD') return 'USD';
  if (cur === '€' || cur === 'يورو' || cur === 'EUR') return 'EUR';
  return cur || 'ILS';
}

function addVaultCurrencyAmount(map: Record<string, number>, currency: string, amount: number): Record<string, number> {
  return { ...map, [currency]: roundMoney(Number(map?.[currency] || 0) + Number(amount || 0)) };
}

function getProvidedVaultFxRate(args: any, currency: string): number | null {
  const rates = args?.exchangeRates || args?.fxRates || {};
  const direct = Number(args?.exchangeRate || args?.fxRate || 0);
  const keyed = Number(rates?.[currency] || rates?.[currency.toLowerCase()] || 0);
  const rate = keyed > 0 ? keyed : direct;
