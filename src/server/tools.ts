...
export async function getCommitments(args: any, userId: string, token: string) {
  ...
  const enriched = commitments.map((c: any) => {
    ...
    const diffMs = Number.isFinite(due.getTime()) ? due.getTime() - now.getTime() : 0;
