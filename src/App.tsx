...
import { getCurrentSalaryCycle, getSalaryCycleForDate, summarizeSalaryCycleTransactions } from './lib/salaryCycle';
...
export default function App() {
  ...
  const [recurringCommitmentCandidates, setRecurringCommitmentCandidates] = useState<any[]>([]);
  ...
  const [dailyFinancialPulses, setDailyFinancialPulses] = useState<any[]>([]);
  const [isDailyPulseGenerating, setIsDailyPulseGenerating] = useState(false);
  ...
  const selectedVaultCycleIdRef = useRef<string>('');
  selectedVaultCycleIdRef.current = selectedVaultCycleId || selectedVaultCycleOption?.cycleId || selectedVaultCycleOption?.id || vaultData?.currentCycle?.cycleId || '';
  ...
  useEffect(() => {
    ...
    const applyCachedDashboardData = async () => {
      const [cachedTx, cachedRep, cachedBudgets, cachedCommitments, cachedSavings, cachedAdvisorPulse, cachedTreasurerProfile, cachedAdvisorAlerts, cachedAdvisorAudit, cachedMarketWatchlist, cachedFinancialScenarios, cachedRecurringCommitmentCandidates, cachedFinancialHabitReports, cachedWeeklyFinancialPlans, cachedAdaptiveBudgetPlans, cachedMonthEndForecasts, cachedDailyFinancialPulses] = await Promise.all([
        ...
        idbGet<any[]>('lkgs_financial_scenarios'),
        idbGet<any[]>('lkgs_recurring_commitment_candidates'),
        idbGet<any[]>('lkgs_financial_habit_reports'),
      ...
      if (Array.isArray(cachedFinancialHabitReports)) setFinancialHabitReports(cachedFinancialHabitReports);
      if (Array.isArray(cachedWeeklyFinancialPlans)) setWeeklyFinancialPlans(cachedWeeklyFinancialPlans);
      if (Array.isArray(cachedAdaptiveBudgetPlans)) setAdaptiveBudgetPlans(cachedAdaptiveBudgetPlans);
      if (Array.isArray(cachedMonthEndForecasts)) setMonthEndForecasts(cachedMonthEndForecasts);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        await fetchDailyFinancialPulsesData(headers);
    ...
    const handleDataWiped = () => {
      ...
      setMarketWatchlist([]);
      setFinancialScenarios([]);
      setRecurringCommitmentCandidates([]);
      setFinancialHabitReports([]);
  ...
  const activeCycleDebtOutflow = Number(activeDashboardSummary?.debtPaymentLiquidityOutflow || 0);
  const cycleSpendableBalance = Math.max(0, Number(activeDashboardSummary?.totalInflow ?? monthIncome) - monthExpense - activeCycleDebtOutflow - activeCycleVaultContribution);
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-slate-950 font-sans text-slate-50 pb-32 selection:bg-emerald-500/30">
          ...
          <div className={`relative overflow-hidden rounded-3xl p-6 shadow-2xl border border-white/10 ${
            ...
            <div className="relative z-10">
              ...
              <div className="mb-6 bg-cyan-500/10 border border-cyan-500/20 rounded-2xl px-3 py-2">
                ...
                <p className="text-[10px] text-slate-500 mt-0.5">يُحسب من دخل ومصروفات الدورة، ولا يخصم سداد دين الدورة نفسها مرتين.</p>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2 text-center mb-3">
                  ...
                  <div className="bg-slate-950/50 border border-slate-800 rounded-2xl p-2.5">
                    ...
                    {Number(dailyFinancialPulses[0]?.summary?.spendingPressureGap || 0) > 0 && <p className="text-[9px] text-amber-200 mt-1">ضغط متوقع {Number(dailyFinancialPulses[0]?.summary?.spendingPressureGap || 0).toLocaleString()} ₪</p>}
                  </div>
                  <div className="bg-slate-950/50 border border-slate-800 rounded-2xl p-2.5">
            ...
            {adaptiveBudgetPlans.length > 0 ? (
              <div>
                <div className="flex items-center justify-between gap-3 mb-3">
                  ...
                  <span className="text-[10px] text-slate-300 bg-black/20 border border-white/10 rounded-full px-2 py-1">
                    {adaptiveBudgetPlans[0]?.applied ? 'مطبقة' : adaptiveBudgetPlans[0]?.status === 'initial_setup' ? 'إعداد أولي' : adaptiveBudgetPlans[0]?.status === 'tightened' ? 'مشددة' : adaptiveBudgetPlans[0]?.status === 'rebalanced_growth' ? 'نمو' : adaptiveBudgetPlans[0]?.status === 'needs_manual_review' ? 'تحتاج مراجعة' : 'متوازنة'}
                  </span>
                </div>
          ...
          <div className="bg-slate-900 border border-purple-500/25 rounded-3xl p-5 shadow-xl">
            ...
            {monthEndForecasts.length > 0 ? (
              <div>
                ...
                <div className="grid grid-cols-3 gap-2 text-center mb-3">
                  <div className="bg-slate-950/50 border border-slate-800 rounded-2xl p-2.5">
                    <p className="text-[10px] text-slate-400 mb-1">هامش بعد الصرف</p>
                ...
                {vaultData?.vaultBalanceByCurrency && Object.keys(vaultData.vaultBalanceByCurrency).length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-1 text-[10px] text-slate-300">
                    {Object.entries(vaultData.vaultBalanceByCurrency).map(([currency, amount]: any) => (
                      <span key={currency} className="px-2 py-0.5 rounded-full bg-slate-800 border border-slate-700">{Number(amount || 0).toLocaleString()} {currency}</span>
                    ))}
                  </div>
                )}
              </div>
      ...
      {/* Savings Vault Modal */}
      {showVault && (
        <div className="mobile-modal-backdrop bg-black/70 backdrop-blur-md z-[105]">
          <div className="mobile-modal-panel bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-4xl flex flex-col p-4 sm:p-6 shadow-2xl relative">
                ...
                <div className="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-3 text-xs">
                  ...
                  <div className="bg-slate-900/70 border border-rose-500/20 rounded-2xl p-3 max-h-44 overflow-auto">
                    ...
                    {(selectedVaultCycleDetails.debtPurchases || []).length ? selectedVaultCycleDetails.debtPurchases.map((tx: any) => {
                      ...
                      return (
                        <div key={tx.id} className="flex justify-between gap-2 border-b border-slate-800 py-1 last:border-0">
                          <span className="text-slate-300">{tx.date?.slice(0,10)} · {tx.category || 'مصروف'}{tx.subcategory ? ` / ${tx.subcategory}` : ''}{tx.creditor || tx.merchant ? ` · دين على ${tx.creditor || tx.merchant}` : ' · دين'}</span>
                          <span className="flex items-center gap-2 font-bold text-rose-200">
                            {needsDebtFix && <button onClick={() => repairSelectedCreditPurchase(tx)} disabled={isVaultCycleLoading} className="px-2 py-1 rounded-lg bg-amber-500/15 border border-amber-500/40 text-amber-200 text-[10px] disabled:opacity-50">ثبّت كدين</button>}
                            {Number(tx.amount || 0).toLocaleString()} ₪
                          </span>
                        </div>
      ...
      {showSavings && (
        <div className="mobile-modal-backdrop bg-black/70 backdrop-blur-md z-[105]">
          <div className="mobile-modal-panel bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-2xl flex flex-col p-4 sm:p-6 shadow-2xl relative">
      ...
      {showScannerResult && (
        <div className="mobile-modal-backdrop bg-black/80 backdrop-blur-md z-[120]">
          <div className="mobile-modal-panel bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-2xl flex flex-col p-4 sm:p-6 shadow-2xl relative">
