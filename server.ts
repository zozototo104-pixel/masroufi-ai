...
function isFinancialToolName(name: string): boolean {
  return ['add_transaction', 'transfer_money', 'pay_debt', 'send_palpay_payment', 'delete_transaction', 'delete_recent_transactions', 'update_transaction', 'repair_misrecorded_credit_purchase', 'repair_duplicate_income', 'repair_duplicate_credit_purchase', 'repair_account_balance_snapshot', 'query_transactions', 'queryTransactions', 'get_recent_transactions', 'getRecentTransactions', 'get_salary_cycle_summary', 'get_savings_vault', 'recalculate_salary_cycle', 'add_savings_vault_adjustment', 'repair_savings_vault_meta'].includes(name);
}
...
function buildFallbackFinancialToolCall(userText: string, clientMessageId: string): FunctionCall | null {
  ...
  const isCreditPurchaseRepair = /(رجع|رجعي|ارجع|أرجع|صحح|صححي|اصلح|أصلح|تعديل|النقص|خصم|خصمت|مش معترف|غير معترف|حاططها|حاطها|مصروفات)/.test(text)
    && /(دين|بالدين|اجل|آجل|على الحساب)/.test(text)
    && (/(نقد|نقدي|كاش|palpay|بال باي|بالباي|الرصيد)/.test(text) || /(مش معترف|غير معترف|حاططها|حاطها|مصروفات)/.test(text));
...
For Arabic/RTL tables, inspect the visual date column on the far right or far left and attach that date to the matching visual row. Build dateMap for every extracted item row that has a visible printed date anywhere aligned with it. If several consecutive rows are visually grouped under one printed date, add a dateMap entry for each affected row. Row numbers in dateMap must be 1-based and match the items array order. If a row has a month but no day, keep date empty and include day only if visible. If a date is not printed in the source, leave date empty; do not infer from upload date, current date, or today's date. If no line items can be broken down, provide a single item with the total amount.`;
      ...
      const validationFailed = error?.reason === 'EXPENSE_IMPORT_NORMALIZATION_FAILED' && error?.preview;
      if (validationFailed) return res.status(422).json({ success: false, ...error.preview });
      const rateLimited = error?.reason === 'GEMINI_RATE_LIMIT_EXCEEDED' || error?.reason === 'GEMINI_KEY_POOL_EXHAUSTED' || isGeminiRateLimitError(error);
      const temporary = !rateLimited && (error?.reason === 'GEMINI_TEMPORARILY_UNAVAILABLE' || isGeminiTemporaryCapacityError(error));
      const keyAttempts = Array.isArray(error?.keyAttempts) ? error.keyAttempts.length : 0;
      res.status(rateLimited ? 429 : temporary ? 503 : 500).json({
        ...
        retryable: temporary,
        keyAttempts,
        keyPoolSize: error?.geminiKeyPoolSize || undefined,
        message: rateLimited
          ? 'حصة أو حد Gemini لتحليل الصور وصل للحد مؤقتاً على المفاتيح المتاحة. انتظر رجوع الحصة أو استخدم Excel/CSV الآن.'
          : temporary
      ...
      const committed = await atomicAddTransactions(
        ...
        prepared.map((row) => ({ ...row.transaction, receiptId })),
        {
          ...
          receiptMeta: {
            ...
            balanceSource: accountBalanceSnap?.exists ? 'accountBalances' : 'missing-accountBalances-blocked',
  ...
  app.post("/api/advisor/habits", authMiddleware, async (req: any, res: any) => {
    try {
      const { analyzeFinancialHabits } = await import('./src/server/tools');
      const token = req.headers.authorization.split('Bearer ')[1];
      res.json(await analyzeFinancialHabits(req.body || {}, req.user.uid, token));
    } catch (e: any) {
      console.error('Advisor habit analysis error:', e.message);
      res.status(500).json({ error: e.message });
    }
  });

  app.get("/api/advisor/habits", authMiddleware, async (req: any, res: any) => {
    try {
      const { getFinancialHabitReports } = await import('./src/server/tools');
      const token = req.headers.authorization.split('Bearer ')[1];
      res.json(await getFinancialHabitReports(req.query || {}, req.user.uid, token));
    } catch (e: any) {
      console.error('Advisor habit reports error:', e.message);
      res.status(500).json({ error: e.message });
    }
  });
  ...
  app.post("/api/savings-vault/repair-meta", authMiddleware, async (req: any, res: any) => {
    try {
      ...
      const token = req.headers.authorization.split('Bearer ')[1];
      res.json(await repairSavingsVaultMeta(req.body || {}, req.user.uid, token));
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
  });

  app.post("/api/savings-vault/repair-misrouted-close", authMiddleware, async (req: any, res: any) => {
    try {
      const { repairMisroutedVaultClose } = await import('./src/server/tools');
      const token = req.headers.authorization.split('Bearer ')[1];
      res.json(await repairMisroutedVaultClose(req.body || {}, req.user.uid, token));
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
  });
  ...
  app.post("/api/account-balance/repair", authMiddleware, async (req: any, res: any) => {
    try {
...
3. في قرارات الشراء، السوق المحلي يعني غزة أولاً ثم فلسطين ثم العالمي كمرجع. استخدم search_local_market للمشتريات المهمة، وخزّن عروض المحلات الموثوقة عبر save_market_offer حتى يبني مصروفي ذاكرة سوق غزة تدريجياً. إذا كان الشراء غير عاجل أو له سعر مستهدف، استخدم create_market_watch_item أو review_market_watchlist حتى يصبح الخبير مراقب أسعار لا مجرد باحث لحظي. قبل أي شراء مهم أو كمالي، استخدم assess_financial_goal_impact لشرح أثره على أهداف الادخار.
...
0.0.1- **إجابات التوضيح**: إذا سألت عن حقل ناقص مثل كاش/PalPay/دين أو الغرض أو الدائن ورد المستخدم بإجابة قصيرة، لا ترد كلامياً فقط ولا تقل سجلت؛ أعد استدعاء الأداة المالية نفسها بالقيم السابقة مع الإجابة الجديدة، وانتظر success=true.
...
0.1.1- **إقفال الشهر وتحويل المتبقي للخزنة**: إذا قال المستخدم "حوّل المتبقي للخزنة" أو "اقفل الشهر" أو "اقفل الدورة" أو "رحّل الفائض للخزنة" فالمطلوب حصراً هو recalculate_salary_cycle مع lockVault=true وcloseCycle=true وtransferToVault=true. لا تستخدم pay_debt ولا تسجل فائض سداد دائن، ولا تنشئ تحويل يدوي منفصل؛ أداة دورة الراتب هي التي تنشئ VAULT_LOCK الذري.
0.2- **الحذف والتعديل والبحث بتاريخ**: إذا كانت العملية المقصودة غير فريدة، لا تخمن أحدث عملية. اطلب تحديدها. التعديل يجب أن يغير نفس السجل ولا ينشئ عملية مالية جديدة. إذا سأل المستخدم "شو في مصروفات بتاريخ كذا" أو "في مشتريات بتاريخ كذا" فاستخدم query_transactions مع date لذلك اليوم، وليس month ولا دورة الراتب، إلا إذا قال صراحة دورة راتب/دورة شهر.
0.2.1- **آخر العمليات المالية**: إذا سأل المستخدم "شو آخر العمليات المالية؟" أو "آخر العمليات اللي سجلناها" أو "آخر مصروفات/قيود" فاستخدم get_recent_transactions فقط، واقرأ الملخص الذي تعيده الأداة. لا تستخدم memory_search لهذا السؤال أبداً، ولا تقل لا يوجد قبل قراءة transactions، ولا تستخدم query_transactions إلا إذا طلب فترة محددة.
...
0.3.1- **حد الصرف الآمن**: إذا سأل المستخدم "كم أقدر أصرف؟" أو "شو الحد الآمن؟" أو "هل وضعي يسمح؟" أو عند إعطاء نبض مالي يومي، استخدم get_safe_spending_limit. لا تعطِ رقماً آمناً من التخمين؛ اعتمد على الأداة لأنها تحمي الالتزامات، الاحتياطي، أهداف الادخار، ونمط الصرف المعتاد.
0.3.2- **مركز تنبيهات الخبير**: إذا سأل المستخدم "شو التنبيهات؟" أو "في مخاطر؟" أو "شو لازم أتابع؟" استخدم get_advisor_alerts. لا تغلق أو تتجاهل تنبيهاً إلا إذا طلب المستخدم ذلك صراحة، وعندها استخدم update_advisor_alert بالإجراء المناسب.
0.3.3- **المدقق المالي الشامل**: إذا طلب المستخدم "دقق حساباتي" أو "افحص وضعي" أو "في أخطاء؟" أو "راجع الديون/الراتب/الالتزامات" استخدم run_financial_audit. ابدأ بنطاق salary_cycle أو recent، ولا تستخدم all إلا بعد تأكيد صريح لأن القراءة واسعة.
0.3.4- **ملف أمين الصندوق الشخصي**: عند نقص بيانات الملف المالي أو عندما يذكر المستخدم راتباً/يوم راتب/احتياطياً/حد دين/أولوية/تفضيل تنبيه، استخدم get_treasurer_profile ثم update_treasurer_profile. اسأل سؤالاً واحداً فقط عن أهم حقل ناقص، ولا تطلب قائمة طويلة دفعة واحدة.
0.3.5- **محاكاة السيناريوهات**: إذا سأل المستخدم "لو صرفت/اشتريت/وفرت/دخلني مبلغ" أو "شو يصير آخر الشهر؟" استخدم simulate_financial_scenario. لا تسجل عملية من السؤال الافتراضي، واذكر baseline وبعد السيناريو وخطة التعويض إن وجدت.
0.3.6- **مدير الاشتراكات والالتزامات المتكررة**: إذا سأل المستخدم عن الاشتراكات/الفواتير/الأقساط المتكررة أو قال "اكتشف المصاريف المتكررة" استخدم detect_recurring_commitments. لا تحول مرشحاً إلى التزام إلا بعد موافقة صريحة أو إذا أعطى المستخدم التزاماً متكرراً واضحاً، وعندها استخدم create_recurring_commitment_from_candidate.
0.3.7- **محرك العادات والأنماط المالية**: إذا سأل المستخدم "وين بصرف أكثر؟" أو "حلل عاداتي" أو "ليش المصروف زاد؟" أو "أي يوم بصرف أكثر؟" استخدم analyze_financial_habits. قارِن بالفترة السابقة، واذكر نمطاً أو اثنين مع سبب ورقم وخطوة عملية، ولا تتهم المستخدم بلا دليل.
0.3.8- **خطة الأسبوع الذكية**: إذا سأل المستخدم "شو أعمل هذا الأسبوع؟" أو "اعطيني خطة" أو "كيف أرتب وضعي؟" أو "كم أوفر وكم أسدد؟" استخدم generate_weekly_financial_recommendations. اعرض الخطة كبنود عملية: أوقف، خفّض، سدّد، حوّل للأهداف، واذكر المبالغ المقترحة والسبب.
0.3.9- **الميزانية المتكيّفة**: إذا سأل المستخدم "عدّل الميزانية" أو "اقترح حدود البنود" أو "خلّي الميزانية أذكى" استخدم generate_adaptive_budget_plan. لا تستخدم apply_adaptive_budget_plan إلا بعد موافقة صريحة لأن التطبيق يغيّر حدود الميزانيات المحفوظة.
0.3.10- **توقع نهاية الشهر**: إذا سأل المستخدم "كيف بخلص الشهر؟" أو "هل حأدخل بعجز؟" أو "كم حيبقى معي؟" أو "شو وضعي آخر الشهر؟" استخدم forecast_month_end_financial_position. اعرض الحالة: فائض/متوازن/ضغط/عجز، مع الأسباب، السقف اليومي، وخطة التصحيح.
0.3.11- **نبض اليوم المالي**: إذا سأل المستخدم "شو أعمل اليوم؟" أو "نبض اليوم" أو "كم أصرف اليوم؟" أو "ممنوع أصرف على شو؟" استخدم generate_daily_financial_pulse. أعطه سقف اليوم، الخطر الأكبر، أهم تذكير، وما لا يجب صرفه اليوم، بدون ادعاء تشغيل تلقائي بالخلفية.
...
3. **التقارير المالية**: إذا طلب المستخدم "تقرير كامل/شامل/مفصل/كل المصروفات" فهذا تقرير كتابي كامل وليس رداً صوتياً مختصراً؛ استخدم generate_report واحفظه في حافظة التقارير، ولا ترفض بسبب كثرة البيانات. استخدم generate_treasurer_report للتحليل مثل: "مصروفات الأولاد الشهر هذا"، "أكثر المصروفات الشهر هذا"، "وين أعلى صرف"، أو ملخص بنود وتوصيات، ولا تقل البيانات كثيرة؛ الأداة ستلخص ضمن حد آمن. استخدم query_transactions فقط إذا طلب قائمة عمليات أو آخر عمليات. تقرير مصروفات شهر 8 يعني دورة راتب شهر 8 27→26 إلا إذا قال صراحة الشهر الميلادي، وعبارة الشهر هذا تعني دورة الراتب الحالية في سياق المصروفات.
...
5. **الدخل والراتب ودورة الراتب**: لا تسجل الراتب كاش تلقائياً؛ اسأل كم نقدي وكم PalPay، أو استخدم allocations إذا أعطى التوزيع. دورة الراتب تبدأ يوم 27 وتنتهي يوم 26؛ راتب 27/6 ومصروفات 27/6 إلى 26/7 تخص راتب/دورة شهر 7. أي سؤال عن "شهر 7" أو "مصروفات يوليو" أو "مصروفات شهر أغسطس" يعني دورة الراتب لذلك الشهر حتى لو لم يقل كلمة دورة. لا تستخدم الشهر الميلادي إلا إذا قال صراحة "الشهر الميلادي" أو أعطى startDate/endDate. عند سؤال مصروفات/دخل شهر معيّن استخدم query_transactions مع period=salary_cycle وmonth. عند سؤال دين شهر معيّن استخدم query_transactions أو get_salary_cycle_summary مع month ولا تستخدم get_balance وحده، لأن المطلوب دين الدورة والمتبقي بعد السداد. الدين النقدي المستلم يزيد التدفق الداخل النقدي لكنه ليس دخلاً حقيقياً ولا مصروفاً ولا فائض خزنة. ممنوع استدعاء add_transaction مرتين لنفس الراتب. إذا قال المستخدم إن الراتب تكرر أو تضاعف، استخدم repair_duplicate_income لإزالة النسخة الزائدة ولا تضف عملية عكسية.
5.1. **حذف آخر عمليات الدين**: إذا قال المستخدم "احذف آخر عملية سداد دين" أو "احذفي آخر تسديد دين" فاستخدم delete_recent_transactions مباشرة مع kind=debt_payment وcount=1. أما إذا قال "احذف آخر عملية دين" أو "احذف آخر مشتريات دين" أو "احذف الدين من عند فلان" بدون كلمة سداد/تسديد، فاستخدم delete_recent_transactions مع kind=credit_purchase وcount=1. لا تبحث كمصروف عادي ولا تقل لا يوجد قبل فحص النوع الصحيح.
5.2. **تاريخ سداد الدين**: سداد الدين العادي بدون تاريخ يسجل بتاريخ اليوم. لكن إذا قال المستخدم "سدد الدين القديم لدورة شهر 8 من رصيد شهر 8" أو "بتاريخ الدين" أو "لحظة الدين" فهذا ليس سداد اليوم؛ استخدم pay_debt مع salaryMonth/fromSalaryCycleBalance/useDebtDate حتى تُسجل عملية السداد داخل دورة الراتب المقصودة، ولا تدخل دورة اليوم/الشهر الحالي.
5.3. **تصحيح دين خُصم من النقدي**: إذا قال المستخدم إن شراء دين خصم من النقدي/PalPay أو سأل "مين يرجع النقص" فاستخدم repair_misrecorded_credit_purchase مع المبلغ والدائن إن توفروا. لا تضف دخل ولا تعمل تحويل يدوي؛ التصحيح يعدل نفس العملية ويرجع النقدي تلقائياً.
6. **قيد واحد ومنطق الأرصدة**: لا تخترع طريقة الدفع. إذا ذكر تفاصيل الشراء ولم يذكر كاش/PalPay/دين فاسأله أولاً: كاش أم PalPay أم دين؟ إذا قال كاش: add_transaction expense account=cash ويخصم النقدي فقط. إذا قال PalPay: add_transaction expense account=palPay ويخصم PalPay فقط. إذا قال "اشتريت من فلان بـ 50 دين/بالدين/على الحساب" فهذا قيد شراء دين واحد فقط: add_transaction type=expense account=debt paymentMethod=debt amount=50 merchant/creditor=فلان transactionType=CREDIT_PURCHASE؛ يزيد الدين فقط ولا يخصم النقدي أو PalPay. لا تستخدم pay_debt لأن pay_debt للسداد فقط. ممنوع معه transfer_money وممنوع تسجيله مرتين. إذا قال "أخذت/استدنت/اقترضت دين 100 من فلان" فهذا ليس دخلاً عادياً وليس شراءً؛ استخدم transfer_money من debt إلى الحساب الذي قال إنه استلم فيه المال. إذا لم يذكر كاش أو PalPay فاسأله ولا تفترض الكاش. هذا يزيد الدين ويزيد الرصيد المستقبل. إذا قال "سددت/دفعت دين" فقط عندها استخدم pay_debt بعد معرفة حساب الدفع cash/palPay، وهذا ينقص الدين وينقص الحساب المدفوع منه.
7. لا تقل "تم تسجيل العملية بنجاح" بل تحدث بعفوية ومختصر.`;
      ...
      const financialToolResults = executedFunctionResponses
        ...
        .filter(Boolean);
      const finalHasCommittedFinancialMutation = executedFunctionResponses.some((r: any) => isFinancialMutationToolName(r.name) && isCommittedFinancialMutationResponse(r.response));
      if (!finalHasCommittedFinancialMutation && looksLikeFinancialWriteIntent(recentUserConversationText) && looksLikeCommittedClaim(replyText || '')) {
...
const GEMINI_LIVE_TOOL_NAMES = new Set([
  // Voice uses the broad financial tool surface again. The immediate Live close
  // was caused by invalid compacted schemas, not by the number of tools. Keep
  // GEMINI_LIVE_TOOLS_MODE=off available as an emergency fallback only.
  'add_transaction',
  'transfer_money',
  'pay_debt',
  'delete_transaction',
...
3.1. أمر شراء مكتمل في الصوت مثل "اشتريت/شريت/دفعت/مصروف" + مبلغ + كاش/PalPay/دين يعني تنفيذ add_transaction فوراً. ممنوع تبدأ بـ query_transactions أو memorySearch وممنوع تقول سجلت أو خصمت قبل نجاح add_transaction بنفس الطلب. أما نية شراء غير عاجلة أو سعر مستهدف فحوّلها إلى create_market_watch_item أو review_market_watchlist بدل تسجيل مصروف. قبل أي شراء مهم أو كمالي، استخدم assess_financial_goal_impact لشرح أثره على أهداف الادخار.
4. إذا رجعت أداة مالية pending أو cloudStoragePending فهذا يعني أن Firestore لم يؤكد الحفظ السحابي. لا تبرره بضعف شبكة المستخدم. قل بصراحة: الحفظ السحابي غير مؤكد، وافحص Firebase/Firestore أو أعد المحاولة.
...
0.7.0.1- **منع النجاح الوهمي**: لا تقل "سجلت" أو "خصمت" إلا إذا كانت نتيجة add_transaction أو transfer_money أو pay_debt لنفس الطلب الحالي فيها success=true و cloudStorageConfirmed=true أو durability=committed. إذا ظهرت postCommitVerificationWarning مع success=true وdurability=committed فهذا تحذير تشخيصي بعد الحفظ وليس فشل حفظ؛ قل تم الحفظ، ويمكن ذكر أن الرصيد تم فحصه/إصلاحه إن لزم. فقط إذا success=false أو durability=pending قل إن الحفظ غير مؤكد.
0.7.1- **إدخال مصروفات أشهر سابقة**: عند نقل مصروفات أشهر سابقة، إذا ذكر المستخدم تاريخاً كاملاً لكل بند فمرره في date. إذا قال فقط "شهر 6/2026" أو "يونيو 2026" بدون يوم لكل عملية، لا تخترع تاريخاً؛ مرر historicalMonth واليوم المذكور، أو اسأل عن اليوم/اطلب منه وضع تاريخ كل بند.
0.8- **V6.1 — السوق الحقيقي المرتبط بالسياق المالي**: عند طلب المستخدم مقارنة سعر سلعة مهمة (جوال، سيارة، إلكترونيات، أو طلب صريح "دورلي/غالي ولا رخيص")، اسأل أولاً عن الموديل/المواصفات والحالة (جديد/مستعمل) إذا لم تُذكر، ثم استخدم search_local_market. ادمج النتيجة مع السياق المالي: الرصيد، متوسط الصرف اليومي، الالتزامات، الموازنة، توقع 30 يوماً. لا تقدم سعر السوق بمعزل عن الواقع المالي للمستخدم. لا تخترع أسعاراً — إذا أعادت الأداة marketUnavailable، قل "لا أستطيع التحقق من السعر حالياً" وتابع النصيحة المالية بدون ادعاء معرفة السوق.
0.9- **V6.1 — نية الشراء vs الشراء المنجز**: فرّق بدقة بين "بدي أشتري/بفكر/شو رأيك" (نية) و"اشتريت" (منجز). النية = استخدم assess_purchase و search_local_market، لا تسجّل عملية. المنجز = اسأل عن طريقة الدفع ثم سجّل بـ add_transaction. الاختلاط بينهما يفسد البيانات المالية.
1- **سؤال الدفع الإلزامي**: عند تسجيل مصروف أو شراء، إذا لم يذكر المستخدم "كاش" أو "بال باي" أو "دين" صراحة، **يُمنع منعاً باتاً** استخدام أداة add_transaction. اسأله فوراً: "هل دفعت كاش أم من محفظة بال باي أم سجلتها ديناً؟".
...
2. **حذف أو إلغاء العمليات (Deletions & Deductions)**:
   ...
   - إذا ذكر المستخدم تاريخاً مثل "27/8" أو "2026-08-27"، مرّر التاريخ في حقل \`date\` أيضاً، خصوصاً للعمليات الظاهرة في تتبع النقدي/PalPay أو دورة الراتب؛ لا تكتفِ بالبحث في آخر العمليات.
   - إذا قال "مصروف نقدي" فهذا يعني غالباً \`account=cash\` وليس اسم التصنيف، لذلك لا تجعله يمنع العثور على التصنيف الحقيقي مثل "طعام ومشتريات منزل".
...
3. **التصنيف الهيكلي الدقيق للعمليات (add_transaction)**:
   ...
   - **درجة الضرورة (necessity)**: 'ضروري' أو 'كمالي'، وتصنيفها مسؤوليتك أنت وفق واقع غزة والحرب وصعوبة الحياة، لا وفق معيار رفاهية عام. غذاء، ماء، دواء، كهرباء، غاز، احتياجات أطفال، مأوى، ومواصلات علاج/عمل = غالباً ضروري. مطاعم، عطور، إكسسوارات، ترفيه، وأجهزة فاخرة = غالباً كمالي. اسأل فقط إذا وصف الشراء غير كافٍ لفهم الحاجة، ولا تسأل المستخدم دائماً "ضروري ولا كمالي".
   - **اكتمال القيد**: قبل add_transaction اجمع amount + type + category + subcategory + paymentMethod. أما necessity فصنّفها أنت وفق واقع غزة عندما الوصف واضح، واسأل سؤالاً قصيراً فقط إذا لم تكفِ تفاصيل الشراء. احفظ merchant إذا ذكره، وإذا كان الشراء من محل/شخص ولم يذكره وكان مهماً للتتبع فاسأله مرة واحدة. املأ purchaseItem بما تم شراؤه وbeneficiary لمن/لأي غرض، لأنهما جزء من id القيد ومنع التكرار الصحيح. notes يجب أن تلخص ما تم شراؤه. التاريخ الحالي يُستخدم تلقائياً ما لم يذكر تاريخاً آخر. عند أي استدعاء لـ add_transaction مرر userText/currentUserText بالنص الأصلي الأخير للمستخدم حتى لا يخلط الخادم بين اسم محل/دائن وبين طريقة الدفع.
   - مثال: "اشتريت أواعي للعيال بـ200" → لا تنفذ فوراً إذا طريقة الدفع غير مذكورة. اسأل كاش/PalPay/دين، ثم سجّل category=الأبناء وsubcategory=ملابس وamount=200 وpurchaseItem=ملابس وbeneficiary=الأبناء وnotes=ملابس للأبناء وmerchant إن توفر وnecessity وفق ظروف المستخدم لا وفق افتراض عام.
4. لا توافق على كل شيء! إذا قال المستخدم إنه يريد شراء هاتف/جهاز/شيء مكلف، استخدم search_local_market فقط ولا تخترع سعراً أو محلاً. إذا وجدت مصدراً حقيقياً أرخص، اعترض بالأرقام والمصدر؛ وإذا لم تجد، قل بصراحة إن السعر غير مؤكد.
5. تذكر الراتب والمواعيد المهمة! استخدم الذاكرة (memory_search) لتعرف متى راتبه. دورة الراتب تبدأ يوم 27 وتنتهي يوم 26؛ راتب 27/6 ومصروفات 27/6 إلى 26/7 تخص دورة شهر 7. أي سؤال عن شهر مثل "مصروفات شهر أغسطس" أو "دين شهر 8" يعني دورة الراتب لذلك الشهر حتى لو لم يقل كلمة دورة، ولا يعني الشهر الميلادي إلا إذا قال "الشهر الميلادي" صراحة. عند سؤال مصروفات/دخل شهر معيّن استخدم query_transactions مع period=salary_cycle وmonth. عند سؤال دين شهر معيّن استخدم query_transactions أو get_salary_cycle_summary مع month حتى يظهر المتبقي بعد السداد، ولا تستخدم get_balance وحده. الدين النقدي المستلم يظهر كتدفق داخل لكنه ليس دخلاً حقيقياً ولا مصروفاً. اسأله: "هل استلمت الراتب؟ وهل ضايلة قيمته ولا تغيرت؟ وفي إضافي هالشهر؟" واحفظ الإجابة في الذاكرة (memory_save).
            ...
            try {
              ...
              if (message.toolCall && message.toolCall.functionCalls) {
                  ...
                  liveFunctionCalls.map(async (call: FunctionCall) => {
                    try {
                      ...
                      if (handler) {
                        ...
                        const existingArgs: any = effectiveCall.args || {};
                        const hasClarificationMerge = Boolean(existingArgs.clarificationReplyText || existingArgs.clarifiedFromReason);
                        const modelProvidedUserText = String(existingArgs.currentUserText || existingArgs.userText || '').trim();
                        ...
                        const liveTextPatch = liveCurrentUserText
                          ? hasClarificationMerge
                            ? { currentUserText: liveCurrentUserText, clarificationReplyText: existingArgs.clarificationReplyText || liveCurrentUserText }
                            : { userText: liveCurrentUserText, currentUserText: liveCurrentUserText }
                          : {};
                        let toolArgs: Record<string, any> = stableOperationId
                          ? { ...existingArgs, operationId: stableOperationId, ...liveTextPatch }
                          : { ...existingArgs, ...liveTextPatch };
                        ...
                        const isBoundedReadTool = isQueryTransactionsCall || effectiveCall.name === 'memory_search' || effectiveCall.name === 'memorySearch';
                        const runTool = async () => {
                        ...
                        rememberLiveFinancialCommit(liveKey, result);
                        return { id: effectiveCall.id || call.id, name: effectiveCall.name, args: toolArgs, requestArgs: toolArgs, response: result };
                      }
                ...
                if (session && isActive) {
                  ...
                  const functionResponsesForModel = sanitizeLiveFunctionResponsesForGemini(normalizedFunctionResponses as any, deterministicReadReply);
                  console.log("Sending Tool Response:", functionResponsesForModel.map((r: any) => ({ id: r.id, name: r.name, success: r.response?.success === true, reason: r.response?.reason || r.response?.error || null, summaryOnly: Boolean(r.response?.summaryOnly), messageChars: String(r.response?.message || '').length, transactionCommitted: Boolean(r.response?.transactionId || r.response?.cloudStorageConfirmed === true || r.response?.durability === 'committed'), normalizedAfterCommit: Boolean(r.response?.canonicalCommittedTransactionId) })));
                  try {
